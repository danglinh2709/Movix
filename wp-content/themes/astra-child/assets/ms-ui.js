(function () {
  ...
  if (openSearch) openSearch.addEventListener('click', () => setSearchOpen(true));
  closeSearchEls.forEach((el) => el.addEventListener('click', () => setSearchOpen(false)));
  ...
  async function runSearch(q) {
    ...
    const res = await fetch((window.MS_UI && MS_UI.ajaxUrl) || '/wp-admin/admin-ajax.php', {
      method: 'POST',
      body: fd,
    ...
    if (data && data.success && data.data) {
      searchResults.innerHTML = data.data.html || '<div class="ms-empty">No results.</div>';
  ...
  if (tabsRoot) {
    tabsRoot.addEventListener('click', (e) => {
      ...
      $$('[data-ms-tab]').forEach((b) => b.classList.toggle('is-active', b === btn));
      $$('[data-ms-pane]').forEach((p) => p.classList.toggle('is-active', p.getAttribute('data-ms-pane') === key));
    });
  }
  ...
  document.addEventListener('click', async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      const status = data && data.success && data.data ? data.data.status : null;
      if (status === 'added') {
